...
// 创建Dump文件  
//   
void CreateDumpFile(char* lpstrDumpFilePathName, EXCEPTION_POINTERS *pException)  
{  
	// 创建Dump文件  
	//  
...
BOOL CSaveDataToDBDlg::OnInitDialog()
{
	...
	for (int j = 0; j< MAX_LOCALTHREADCOUNT; j++)
	{
...
void CSaveDataToDBDlg::SafeStatuToDB( void )
{
	...
	while(!m_bExit)
	{
		WriteDlgLog("SafeStatuToDB begin.");
		if(m_bExit)
		{
			WriteDlgLog("Leave SafeStatuToDB function.");
			break;
		}
...
void CSaveDataToDBDlg::OnBnClickedButton1()
{
	...
	if (m_bLocalDBEnable)
	{
		...
		m_hReadLocal = (HANDLE)_beginthreadex( NULL, 0, &ThreadReadLocal, this, 0, NULL );
		m_hSaveLocal = (HANDLE)_beginthreadex( NULL, 0, &ThreadSaveLocal, this, 0, NULL );
		for(int i = 0; i<MAX_LOCALTHREADCOUNT; i++)
		{
